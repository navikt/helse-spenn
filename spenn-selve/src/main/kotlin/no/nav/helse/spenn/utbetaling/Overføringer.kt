...
internal class Overføringer(
    ...
    override fun onPacket(
        ...
        if (oppdrag.erKvittert()) {
            ...
            sikkerLogg.info(
                "oppdaterer ikke status for utbetaling $utbetalingId fagsystemId=$fagsystemId status=$status $beskrivelse",
        ...
        sikkerLogg.info(
            "oppdaterer status for utbetaling $utbetalingId fagsystemId=$fagsystemId status=$status $beskrivelse",
            keyValue("fødselsnummer", fødselsnummer),
        )
        oppdragDao.oppdaterOppdrag(avstemmingsnøkkel, utbetalingId, fagsystemId, status)

        oppdragDao
            .hentBehovForOppdrag(utbetalingId, fagsystemId)
            ?.apply {
                this["@løsning"] =
                    mapOf(
                        behovnavn to
