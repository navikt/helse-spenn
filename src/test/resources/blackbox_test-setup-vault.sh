...
vault write postgresql/preprod-fss/config/helse-spenn-oppdrag plugin_name=postgresql-database-plugin allowed_roles=helse-spenn-oppdrag-admin,helse-spenn-oppdrag-user connection_url=postgresql://{{username}}:{{password}}@${_postgresHost}?sslmode=disable username=${_postgresUsername} password=${_postgresPassword}
