...
internal class DataSourceBuilder(env: Map<String, String>) {
    private val databaseName = env["DATABASE_NAME"] ?: env["DATABASE_SPENN_OPPRYDDING_DEV_DATABASE"]
    ...
    private val hikariConfig = HikariConfig().apply {
        jdbcUrl = env["DATABASE_JDBC_URL"] ?: String.format(
            ...
            requireNotNull(env["DATABASE_HOST"] ?: env["DATABASE_SPENN_OPPRYDDING_DEV_HOST"]) { "database host must be set if jdbc url is not provided" },
            requireNotNull(env["DATABASE_PORT"] ?: env["DATABASE_SPENN_OPPRYDDING_DEV_PORT"]) { "database port must be set if jdbc url is not provided" },
            requireNotNull(databaseName) { "database name must be set if jdbc url is not provided" },
            (env["DATABASE_USERNAME"] ?: env["DATABASE_SPENN_OPPRYDDING_DEV_USERNAME"])?.let { "?user=$it" } ?: "")

        (env["DATABASE_USERNAME"] ?: env["DATABASE_SPENN_OPPRYDDING_DEV_USERNAME"])?.let { this.username = it }
        (env["DATABASE_PASSWORD"] ?: env["DATABASE_SPENN_OPPRYDDING_DEV_PASSWORD"])?.let { this.password = it }
    ...
    init {
        if (!shouldGetCredentialsFromVault) {
            if (!env.containsKey("DATABASE_JDBC_URL")) {
                checkNotNull(env["DATABASE_USERNAME"] ?: env["DATABASE_SPENN_OPPRYDDING_DEV_USERNAME"]) { "username must be set when vault is disabled" }
                checkNotNull(env["DATABASE_PASSWORD"] ?: env["DATABASE_SPENN_OPPRYDDING_DEV_PASSWORD"]) { "password must be set when vault is disabled" }
            }
        } else {
